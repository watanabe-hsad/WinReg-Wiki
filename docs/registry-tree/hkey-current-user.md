...
| `Software\Microsoft\Windows\CurrentVersion\Explorer` | Explorer / Shell 用户行为入口。 |
| `Software\Microsoft\Windows\CurrentVersion\Explorer\UserAssist` | Explorer 相关程序交互记录。 |
| `Software\Microsoft\Windows\CurrentVersion\Explorer\RunMRU` | Win+R 输入历史。 |
...
| `Software\Microsoft\Terminal Server Client` | MSTSC 客户端连接历史。 |
| `Software\Microsoft\Windows\CurrentVersion\Run` | 用户级登录启动项。 |
| `Software\Microsoft\Command Processor` | `cmd.exe` 的 `AutoRun` 配置。 |
| `Environment` | 用户级环境变量。 |
| `Software\Microsoft\Windows\CurrentVersion\Internet Settings` | 用户代理、PAC、ZoneMap、WinINet 设置。 |
